...
function help(args, command)
{
    const helloHelp = 'Say hello! Usage `!hello`';
        ...
        const downtimeHelp = 'Downtime command. Inform users of upcoming downtime. Locked to bot operators only. Can set downtime **at** a specific time or downtime **in** a certain amount of time, set `at` or `in` as the type. Usage `!downtime set [type] [time] [type-specific options] [status]` Statuses: `online`, `dnd`, `idle`.\n__**in**__\nSpecify downtime in minutes [m] or hours [h]. Example usage `!downtime set in 5 m idle`\nSpecify downtime at a specific time in 12 or 24 hour time. If 12 hour time, provide PM or AM. Provide timezone. Example usage `!downtime set at 12:35 PM EST idle`'
        if (args.length == 0)
        {
        ...
        else if (args.length > 0) 
        {
            ...
            case "setstatus":
                ...
                {
                        const downtimeHelpEmbed = new Discord.MessageEmbed()
                        .setColor('#0099ff')
                        .setTitle('Specific Command Help - Operator only')
                        .setAuthor('devbot')
                        .setDescription(downtimeHelp)
                        command.author.send(downtimeHelpEmbed).catch(error => {
                            command.channel.send(`<@${command.author.id}> Your DMs are closed. How to fix: https://support.discord.com/hc/en-us/articles/217916488-Blocking-Privacy-Settings`)
                        })
                }
                else
                {
